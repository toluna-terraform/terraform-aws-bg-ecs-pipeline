...
module "code-build" {
  ...
  buildspec_file                        = templatefile("buildspec-build.yml.tpl", { IMAGE_URI = local.image_uri, DOCKERFILE_PATH = var.dockerfile_path, IMAGE_REPO_NAME = var.ecr_repo_name, ADO_USER = data.aws_ssm_parameter.ado_user.value, ADO_PASSWORD = data.aws_ssm_parameter.ado_password.value })
  depends_on = [
    aws_s3_bucket.codepipeline_bucket,
  ]
}
